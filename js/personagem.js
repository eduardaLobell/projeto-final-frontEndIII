...
async function montarCardPersonagem(personagem) {
    ...
    console.log(personagem)
        ...
        detalhamentoPersonagem.innerHTML = `
            <dt class = 'cinzinha'>Última localização conhecida:</dt>
            <dd>${personagem.location.name}</dd>

            <dt class = 'cinzinha'>Visto a última vez em:</dt>
            <dd>${nomeEpisodio}</dd>

            <dt class = 'cinzinha'>Planeta originário</dt>
            <dd>${personagem.origin.name}</dd>
            
            <dt class = 'cinzinha'>Gênero:</dt>
...
function montarFooter (episodios, localizacoes, totalPersonagens) {
    ...
    divColP.setAttribute('class', 'col-12 col-md-2  offset-md-3 verdinho')
    // divColP.innerText = `PERSONAGENS: ${totalPersonagens}`
    divColP.innerHTML = `<p>PERSONAGENS: <span id='branquinho'>${totalPersonagens}</span></p>` 
    
    const divColL = document.createElement('div')
    divColL.setAttribute('class', 'col-12 col-md-2   verdinho')
    // divColL.innerText = `LOCALIZAÇÕES: ${localizacoes}`
    divColL.innerHTML = `<p>LOCALIZAÇÕES: <span id='branquinho'>${localizacoes}</span></p>`

    const divColE = document.createElement('div')
    divColE.setAttribute('class', 'col-12 col-md-2  verdinho')
    // divColE.innerText = `EPISÓDIOS: ${episodios}`
    divColE.innerHTML = `<p>EPISÓDIOS: <span id='branquinho'>${episodios}</span> </p>`
    ...
    divCol2.innerHTML = `<p class='verdinho'>Desenvolvido por: <span id='branquinho'>Eduarda Löbell da Silva</span> em 2023</p>`


    // const divContainer = document.createElement('div')
    // divContainer.setAttribute('class', 'container-fluid d-flex w-100')

    // const rowDesenvolvido = document.createElement('div')
    // rowDesenvolvido.setAttribute('class', 'row desenvolvido justify-content-center')

    // const divColDesenvolvido = document.createElement('div')
    // divColDesenvolvido.setAttribute('class', 'col-12 d-flex align-items-end verdinho')
    // divColDesenvolvido.innerHTML = `Desenvolvido por: <span id='branquinho'> Eduarda Löbell </span> em 2023`
    ...
    // rowDesenvolvido.appendChild(divColDesenvolvido)
    // divContainer.appendChild(rowDesenvolvido)
